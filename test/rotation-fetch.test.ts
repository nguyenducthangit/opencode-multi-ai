...
describe("createRotationFetch", () => {
  ...
  it("retryOnClientError: one-shot body rewrite on same account, no rotation", async () => {
    const adapter = makeAdapter({
      retryOnClientError(ctx, init) {
        if (ctx.status !== 400 || !ctx.bodyText?.includes("service_tier")) {
          return undefined;
        }
        if (!init || typeof init.body !== "string") return undefined;
        const parsed = JSON.parse(init.body) as Record<string, unknown>;
        if (!("service_tier" in parsed)) return undefined;
        const next = { ...parsed };
    ...
    fetchSpy
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ detail: "Unsupported service_tier: fast" }), {
          status: 400,
        }),
      )
      .mockResolvedValueOnce(new Response("ok", { status: 200 }));
    ...
    const custom = createRotationFetch(adapter, manager);
    const res = await custom(ENDPOINT, {
      method: "POST",
      body: wireBody,
    });
    expect(res.status).toBe(200);
    expect(fetchSpy).toHaveBeenCalledTimes(2);
    // Same account retried — no rotation to a sibling.
    expect(bearerOf(fetchSpy.mock.calls[0]![1])).toBe("Bearer tok-a0");
    expect(bearerOf(fetchSpy.mock.calls[1]![1])).toBe("Bearer tok-a0");
    expect(manager.marks.touched).toEqual(["a0"]);
    ...
    expect(retried).not.toHaveProperty("service_tier");
  });

  it("retryOnClientError: surfaces error unchanged when hook declines", async () => {
    const hook = vi.fn(() => undefined);
    const adapter = makeAdapter({ retryOnClientError: hook });
    const manager = makeManager([
      { accountId: "a0" },
      { accountId: "a1" },
    ]);
